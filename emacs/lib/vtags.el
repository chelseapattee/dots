...
(defvar vtags-prefix-matching nil)
...
(defun find-tag-tag (string)
  ...
  (let* ((completion-ignore-case (if (memq tags-case-fold-search '(t nil))
    ...
    (setq vtags-prefix-matching nil)
    (if (equal spec "")
...
Returns the number of matched lines."
  ...
  (if (equal "0" (tagfileinfo-sorted tfi))
    ...
    (let ((vtags-look-buf (get-buffer-create "*Vtags-Look-Buffer*")) ; scratch buffer
          (blksize (* 1024 128)) ; big enough to hold all matching entries
          (default-chunk-size (* 1024 128))   ; twice the length of longest line
          (max-block 0)
          (min-block 0)
          (mid 0)
          (beg 0)
          ...
          (if (not (looking-at "\n"))
            ...
            (when (< tag-length (length tag-line))
              ...
              (if (string-lessp tag tmp-string)
                ...
                (when (and (string-equal tag tmp-string)
                           (or vtags-prefix-matching
                               (string-equal "\t" (substring tag-line
                                                      tag-length
                                                      (+ tag-length 1)))))
                  (setq match-count (1+ match-count))
